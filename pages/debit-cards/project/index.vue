    ...
    <app-table class="mb-4">
      ...
      <app-t-body>
        <app-table-row v-for="(item, index) of projects.data" :key="index">
          ...
          <app-table-cell>
            <app-link :to="`/debit-cards/project/${item.id}`">{{ item.title }}</app-link>
          </app-table-cell>
...
export default {
  ...
  async fetch({ store }) {
    ...
    await store.dispatch('debit-cards/fetchProjects', { per_page: perPageOptions[store.getters["app/perPage"]].id });
  },
  data() {
    return {
      page: 1,
  ...
  methods: {
    async reFetch(page) {
      ...
      await this.$store.dispatch('debit-cards/fetchProjects', {
  ...
  computed: {
    projects() {
      return this.$store.getters["debit-cards/projects"]
    },
    perPage() {
