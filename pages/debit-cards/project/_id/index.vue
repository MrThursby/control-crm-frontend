<template>
  <app-container>
    <breadcrumbs-list class="mb-8">
      ...
      <breadcrumbs-item to="/debit-cards/project">Проекты</breadcrumbs-item>
      <breadcrumbs-item :to="`/debit-cards/project/${$route.params.id}`">Проект #{{ $route.params.id }}</breadcrumbs-item>
    </breadcrumbs-list>
    ...
    <info-list class="mb-6">
      ...
      <div class="w-1/3">
        ...
        <info-item name="Добавлено">{{ $moment(project.created_at) }}</info-item>
      </div>
    </info-list>
...
export default {
  ...
  async fetch({ store, params }) {
    await store.dispatch('debit-cards/fetchProject', params.id)
  },
  computed: {
    project() {
      return this.$store.getters["debit-cards/project"]
    }
  },
