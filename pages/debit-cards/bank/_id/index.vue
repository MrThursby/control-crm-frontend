<template>
  <app-container>
    <breadcrumbs-list class="mb-8">
      ...
      <breadcrumbs-item to="/debit-cards/bank">Банки</breadcrumbs-item>
      <breadcrumbs-item :to="`/debit-cards/bank/${$route.params.id}`">Банк #{{ $route.params.id }}</breadcrumbs-item>
    </breadcrumbs-list>
    ...
    <info-list class="mb-6">
      ...
      <div class="w-1/3">
        ...
        <info-item name="Добавлено">{{ $moment(bank.created_at) }}</info-item>
      </div>
    </info-list>
...
export default {
  ...
  async fetch({ store, params }) {
    await store.dispatch('debit-cards/fetchBank', params.id)
  },
  computed: {
    bank() {
      return this.$store.getters["debit-cards/bank"]
    }
  },
