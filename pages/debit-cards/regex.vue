<template>
  <app-container>
    ...
    <app-table class="mb-4">
      ...
      <app-t-body>
        <app-table-row v-for="(item, index) of regex.data" :key="index">
          ...
          <app-table-cell>{{ item.regex }}</app-table-cell>
...
export default {
  ...
  async fetch({ store }) {
    ...
    await store.dispatch('debit-cards/fetchRegex', { per_page: perPageOptions[store.getters["app/perPage"]].id });
  },
  data() {
    return {
      page: 1,
  ...
  methods: {
    async reFetch(page) {
      ...
      await this.$store.dispatch('debit-cards/fetchRegex', {
