...
export default {
  ...
  async fetch({ store }) {
    ...
    await store.dispatch('debit-cards/fetchProviders', { per_page: perPageOptions[store.getters["app/perPage"]].id });
  },
  data() {
    return {
      page: 1,
  ...
  methods: {
    async reFetch(page) {
      ...
      await this.$store.dispatch('debit-cards/fetchProviders', {
