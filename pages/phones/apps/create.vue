<template>
  <app-container>
    <breadcrumbs-list class="mb-8">
      ...
      <breadcrumbs-item to="/phones/apps/create">
        Добавление нового App ID
      </breadcrumbs-item>
    </breadcrumbs-list>
...
export default {
  ...
  methods: {
    async submit() {
      ...
      await this.$axios.$post('/api/admin/apps', formData)
        .then(r => {
          this.$router.push(`/phone-numbers/apps/${r.data.id}`)
        })
        .catch(e => {
