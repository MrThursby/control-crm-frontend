  ...
  <app-container>
    ...
    <info-list class="mb-6">
      ...
      <info-col>
        ...
        <info-item name="App ID">{{ api_key.app.number }}</info-item>
        <info-item name="Добавлено">{{ $moment(api_key.created_at).format('LLL') }}</info-item>
      </info-col>
    </info-list>

    <app-btn v-if="isAbleTo('api-keys-update')"
             @click="$router.push(`/phones/api-keys/${$route.params.id}/edit`)"
             class="mr-4">Редактировать</app-btn>
    <app-btn v-if="isAbleTo('api-keys-delete')" danger>Удалить</app-btn>
  </app-container>
</template>
