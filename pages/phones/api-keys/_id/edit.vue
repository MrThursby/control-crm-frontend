<template>
  <app-container>
    <breadcrumbs-list class="mb-8">
      ...
      <breadcrumbs-item :to="`/phones/api-keys/${$route.params.id}`">
        API-ключ #{{$route.params.id}}
      </breadcrumbs-item>
      <breadcrumbs-item :to="`/phones/api-keys/${$route.params.id}/edit`">
        Редактировать
      </breadcrumbs-item>
    </breadcrumbs-list>
...
export default {
  ...
  mounted() {
    this.form.api_key = this.item.api_key

    let app_id = this.apps.data.findIndex(app => app.id === this.item.app.id)
    app_id = app_id === -1 ? 0 : app_id
  ...
  computed: {
    ...mapGetters({
      apps_getter: 'phones/apps/paginator',
      item: 'phones/api-keys/item',
    }),
    apps() {
      if(this.apps_getter.data.findIndex(app => app.id === this.item.app.id) === -1){
        let apps = JSON.parse(JSON.stringify(this.apps_getter))
        apps.data = [this.item.app, ...apps.data]
        return apps
      }
      return this.apps_getter
    }
  },
  components: {
    AppFormGroupError,
