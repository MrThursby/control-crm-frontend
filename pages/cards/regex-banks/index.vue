  ...
  <app-container>
    ...
    <page-filters class="flex flex-wrap">
      ...
      <page-filters-item v-if="isAbleTo('regex-banks-create')">
        <app-btn @click="$router.push('/cards/regex-banks/create')" class="w-full">Добавить</app-btn>
      </page-filters-item>
    </page-filters>
    ...
    <app-table class="mb-4">
      ...
      <app-t-body>
        <app-table-row v-for="(item, index) of regex_banks.data" :key="index">
          ...
          <app-table-cell>
            <app-link :to="`/cards/regex-banks/${item.id}`">{{ item.regex }}</app-link>
          </app-table-cell>
...
export default {
  ...
  async fetch({ store }) {
    ...
    await store.dispatch('cards/regex-banks/fetchPaginator', {
  ...
  methods: {
    async reFetch(page) {
      ...
      await this.$store.dispatch('cards/regex-banks/fetchPaginator', {
  ...
  computed: {
    ...mapGetters({
      regex_banks: 'cards/regex-banks/paginator',
