<template>
  <app-container>
    ...
    <page-filters class="flex flex-wrap">
      ...
      <page-filters-item v-if="isAbleTo('banks-create')">
        <app-btn @click="$router.push('/cards/banks/create')" class="w-full">Добавить</app-btn>
      </page-filters-item>
    </page-filters>
    ...
    <app-table class="mb-4">
      ...
      <app-t-body>
        <app-table-row v-for="(item, index) of banks.data" :key="index">
          ...
          <app-table-cell>
            <app-link :to="`/cards/banks/${item.id}`">{{ item.title }}</app-link>
          </app-table-cell>
...
export default {
  ...
  methods: {
    async reFetch(page) {
      ...
      await this.$store.dispatch('cards/banks/fetchPaginator', {
