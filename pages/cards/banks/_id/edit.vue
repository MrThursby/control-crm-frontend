<template>
  <app-container>
    <breadcrumbs-list class="mb-8">
      ...
      <breadcrumbs-item :to="`/cards/banks/${$route.params.id}`">
        Банк #{{ $route.params.id }}
      </breadcrumbs-item>
      <breadcrumbs-item :to="`/cards/banks/${$route.params.id}/edit`">
        Редактировать
      </breadcrumbs-item>
    </breadcrumbs-list>
...
export default {
  ...
  mounted() {
    ...
    this.form.api_name = this.item.api_name
  },
  computed: {
    ...mapGetters({
      item: 'cards/banks/item'
    })
  },
  methods: {
    async submit() {
