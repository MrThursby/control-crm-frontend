<template>
  ...
  >
    ...
    <div :class="modalClass" class="mx-4 bg-dark rounded-md z-40">
      ...
      <div class="p-4">
        <app-btn v-if="ok" @click="$emit('ok')">{{ ok }}</app-btn>
        <app-btn @click="$emit('close')">{{ close || 'Отмена' }}</app-btn>
      </div>
    </div>
  </div>
</template>
...
export default {
  name: "AppModal",
  components: {AppBtn},
  props: ['title', 'ok', 'modalClass', 'close']
}
</script>
